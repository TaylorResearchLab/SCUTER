...
# Protocol: add the two-level ledger model and make validation claim-specific.
path = "protocol/01.work-contract.md"
text = read(path)
anchor = "Until the complete primary text is available and inspected, the reference remains Provisional, inaccessible, or unverified and cannot be used as evidentiary support for a project claim.\n\nSupporting references are admitted with the following protocol:\n"
ledger = dedent('''\
Until the complete primary text is available and inspected, the reference remains Provisional, inaccessible, or unverified and cannot be used as evidentiary support for a project claim.
...
Supporting references are admitted with the following protocol:
''')
text = replace_once(text, anchor, ledger, "protocol ledger insertion")
...
''')
text = replace_once(text, old_steps, new_steps, "protocol validation steps")

old_row = "| Citation ledger | One canonical identifier per work, the statement each reference supports, validation status, who checked and when, and dispositions of candidates not retained | Structured database or a delimited file in the manuscript repository |"
new_row = "| Citation Validation Ledger | One source record per primary work with complete-primary-text location and inspection provenance; within each source record, proposition-specific claim-evidence mappings with locators, support notes, caveats, review, and User disposition | Structured database with source pages or an equivalently structured file in the manuscript repository |"
text = replace_once(text, old_row, new_row, "protocol durable-record row")

start = text.index("The citation ledger is a project record and is not a bibliography.")
end = text.index("\n\nFor this directive, the unit of work is the material action;", start)
replacement = dedent('''\
...
# Skill entry point: concise operational form.
path = "skills/scuter/SKILL.md"
text = read(path)
old = "When uncertain whether an external scientific statement requires citation, cite and validate rather than invoke common knowledge."
new = old + " Record validation in the existing Citation Validation Ledger: one source record per primary work, with separate proposition-specific claim-evidence mappings containing the exact claim, evidence locator, concise support note, caveat, checks, and User disposition. Source-level access or support for one proposition never validates other claims citing the same source."
text = replace_once(text, old, new, "skill ledger rule")
write(path, text)

# Implementation guide: explain the two-level ledger near citation practice.
path = "docs/implementation-guide.md"
text = read(path)
anchor = "Until supplied and inspected, the reference remains Provisional, inaccessible, or unverified and cannot support a project claim.\n\nSupply the Skill"
insertion = dedent('''\
Until supplied and inspected, the reference remains Provisional, inaccessible, or unverified and cannot support a project claim.
...
''')
text = replace_once(text, marker, section + marker, "notion ledger section")
write(path, text)

# Review guide: force reviewers to evaluate mappings rather than paper-level labels.
path = "docs/record-and-review.md"
text = read(path)
marker = "Review requests and approvals describe current routing and endorsement."
add = dedent('''\
...
''')
text = replace_once(text, marker, add + marker, "review ledger rule")
write(path, text)

# Regression test for the high-definition ledger contract.
test = dedent('''\
import unittest
from pathlib import Path
...
if __name__ == "__main__":
    unittest.main()
''')
Path("tests/test_citation_ledger_contract.py").write_text(test)
